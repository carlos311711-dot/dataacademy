...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let syllabusData = [];
    let computedCourses = [];
    let activeCourseIds = []; // Arranca en master view, usuario selecciona lo que quiere
    let conflictsList = [];
    ...
    async function loadData() {
        try {
            const [coursesRes, syllabusRes] = await Promise.all([
                fetch('/data/cursos.csv').then(res => res.text()),
                fetch('/data/temario.csv').then(res => res.text())
            ]);
    ...
    function parseCSV(text) {
        ...
        const headers = lines[0].map(h => h.trim().replace(/^ï»¿/, "")); // Limpiar BOM
        const result = [];
    ...
    function renderCalendarGrid(year, month) {
        ...
        for (let d = 1; d <= numDays; d++) {
            ...
            if (classesToday.length > 0) {
                ...
                if (isConflictDay) {
                    // CÃ‰LULA EN AMARILLO DE CRUCE DE HORARIOS
                    ...
                    classesToday.forEach(c => {
                        tooltipContent += `
                            <div class="multi-tooltip-row">
                                <span class="bullet-${c.colorClass}">â—</span>
    ...
    function renderMasterCoursesTable() {
        ...
        activeCCs.forEach(cc => {
            ...
            tr.querySelector('.btn-view-course').addEventListener('click', () => {
                // Al hacer clic, seleccionamos ÃšNICAMENTE este curso para ver su syllabus individual
